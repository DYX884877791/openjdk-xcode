...
/**
 * URLClassLoader扩展自SecureClassLoader，这个类加载器可以用指向jar包或者文件夹的URL来加载类或者其他资源，
 * URL路径以“/”结束表示该URL指向一个文件夹，否则指向一个jar包。
 *
 * 创建此URLClassLoader实例的线程的AccessControlContext会被用做加载类或者资源的安全控制。
 *
 *  URLClassLoader提供了findResource(final String name)和findClass(final String name)的具体实现，
 * 这两方法在ClassLoader中前者返回null，后者抛出ClassNotFoundException异常
 *
 * 除此之外，URLClassLoader改写了原有的getResourceAsStream(String name)和Enumeration<URL> findResources(final String name)的实现
 *
...
public class URLClassLoader extends SecureClassLoader implements Closeable {
     ...
     * URLClassLoader中的类和其他资源的搜索和读取都由sun.misc.URLClassPath实现
     */
    ...
    /**
     * URLClassLoader的构造方法，URLClassLoader的父类的构造方法的基础上增加了一个必填的参数URL[]
     *
     *
     * 可选的ClassLoader用于指定父类加载器，如果没有默认使用系统类加载器；另外两个可选的URLStreamHandlerFactory和
     * AccessControlContext都是用于构造URLClassPath，AccessControlContext如果没有则使用当前线程的AccessControlContext，
     * 因为构造URLClassPath时URL[]和AccessControlContext是必须的参数
     *
     *
