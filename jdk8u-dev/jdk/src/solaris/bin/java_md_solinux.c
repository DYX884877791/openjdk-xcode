 ...
 * 其他情况均返回0。
 *
 * 了解了RequiresSetenv函数的内部执行过程后就可以构造返回1的条件：将jdk安装目录拷贝到home目录下，记为private，
 * 然后在bash里运行LD_LIBRARY_PATH=~/private/jre/lib/amd64/server _JAVA_LAUNCHER_DEBUG=1 java -version即可使返回值为1。
...
CreateExecutionEnvironment(int *pargc, char ***pargv,
   ...
   */
     ...
     *
     * SetExecname函数使用readlink函数和/proc/self/exe将可执行文件（如java、javac等命令）的绝对路径
     * 如/usr/lib/jvm/java-1.8.0-openjdk-1.8.0.191.b12-1.el7_6.x86_64/jre/bin/java
     * 保存到全局变量execname中，后面查找JRE路径时会用到；
     *
     */
    SetExecname(*pargv);
    ...
    /* Check data model flags, and exec process, if needed */
    {
      ...
      int  argc         = *pargc;
      char **argv       = *pargv;
      // 变量running表示当前机器的数据模型，其值是CURRENT_DATA_MODEL，这个宏定义在文件jdk/src/share/bin/java.h中，
      // 其值是 (CHAR_BIT * sizeof(void*))，CHAR_BIT是定义在limits.h中的宏，表示一个char类型的比特数，sizeof(void*)则是void类型指针的大小；
      ...
      if (wanted == running) {
         ...
         * ReadKnownVMs函数读取JVM配置文件jvm.cfg，其路径是JRE目录/lib/处理器架构/jvm.cfg。这个配置文件每行的含义是如果某一标识符后面是KNOWN，
         * 那么与该标识符同名的目录里包含JVM动态库，以下述文件内容为例，server一行是KNOWN，那么JRE目录/lib/处理器架构/server/libjvm.so是一个JVM动态库。
         * -server KNOWN
         * -client IGNORE
         */
         ...
         * 确定当前JVM类型，先判断是否通过-J、-XXaltjvm=或-J-XXaltjvm=参数指定，如果没有，则读取JVM.cfg文件中配置的第一个类型；
         */
...
#ifdef SETENV_REQUIRED
        /*
         * RequiresSetenv函数测试是否需要设置环境变量，若不需要则结束，命令行选项中的-d64和-d32会在后续ParseArguments函数中被过滤；
         * 否则意味着LD_LIBRARY_PATH内有其他的JVM动态库，但它们并不是我们想用的，所以为LD_LIBRARY_PATH环境变量设置新值为GetJVMPath函数
         * 返回的JVM动态库所在目录:JRE目录/lib/处理器架构:JRE目录/../lib/处理器架构:$LD_LIBRARY_PATH，
         * 接着使用execve函数和新的LD_LIBRARY_PATH环境变量执行原可执行文件（execv那个else分支感觉到不了），但命令行选项去除了-J-d64、-d64、-J-d32和-d32。
         */
...
/*
 * GetJVMPath函数根据CheckJvmType函数返回的JVM类型查找JVM动态库，
 * 以Linux为例
 * JVM动态库的绝对路径就是JRE目录/lib/处理器架构/JVM类型/libjvm.so，windows就是JVM.dll文件；
 *
...
GetJREPath(char *path, jint pathsize, const char * arch, jboolean speculative)
{
    ...
    // GetApplicationHome函数得到可执行文件（如java）所在的应用目录（可执行文件所在bin目录的父目录），
    // 比如Linux下 /usr/lib/jvm/java-1.8.0-openjdk-1.8.0.191.b12-1.el7_6.x86_64/jre/bin/java的应用目录
    // 是/usr/lib/jvm/java-1.8.0-openjdk-1.8.0.191.b12-1.el7_6.x86_64/jre；
    if (GetApplicationHome(path, pathsize)) {
        /* Is JRE co-located with the application?
         * libjava字符数组保存JRE动态库的绝对路径，JRE动态库的文件名由JAVA_DLL宏定义，
         * 在Windows下是java.dll，在Linux下是libjava.so，Mac OS X下是libjava.dylib
         */
        JLI_Snprintf(libjava, sizeof(libjava), "%s/lib/%s/" JAVA_DLL, path, arch);
        /*
         * 如果JRE动态库文件应用目录/lib/处理器架构/libjava.so存在，那么应用目录就是JRE目录；
         * 否则如果文件应用目录/jre/lib/处理器架构/libjava.so存在，那么应用目录/jre就是JRE目录；如果以上两处都没有JRE动态库文件则报错。
         */
...
/*
 * 加载libjvm动态链接库，从中获取JNI_CreateJavaVM，JNI_GetDefaultJavaVMInitArgs和JNI_GetCreatedJavaVMs三个函数的实现，
 * 其中JNI_CreateJavaVM是JVM初始化的核心入口，具体实现在hotspot目录中
 */
jboolean
LoadJavaVM(const char *jvmpath, InvocationFunctions *ifn)
{
    ...
    /*
     * 把动态链接库文件中定义的函数JNI_CreateJavaVM和JNI_GetDefaultJavaVMInitArgs绑定到InvocationFunctions变量的CreateJavaVM和GetDefaultJavaVMInitArgs函数指针变量上；
     */
...
/*
 * 通过JVMInit->ContinueInNewThread->ContinueInNewThread0->pthread_create创建了一个新的线程，执行JavaMain函数，主线程pthread_join该线程，在JavaMain函数中完成虚拟机的初始化和启动。
 */
int
