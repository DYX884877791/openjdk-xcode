...
static void *java_start(Thread *thread) {
  ...
  // handshaking with parent thread
  {
    ...
    // 等待唤醒后，执行 run 方法
  // 主要是调用 Thread 的 run 方法，在 thread.cpp 中 JavaThread::run 方法最终调用了 thread_main_inner 方法：
...
// 并唤醒startThread_lock等待的线程。
// 这时创建的子线程就会开始正常执行，调用JavaThread的run方法
...
void os::pd_start_thread(Thread* thread) {
  ...
  // 就可以唤醒`static void *java_start(Thread *thread)`中
