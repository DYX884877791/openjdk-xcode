...
/*
 * Symbol类的定义位于oops/symbol.hpp中，Symbol表示一个规范化的字符串形式的描述符，如方法Object m(int i, double d, Thread t) {...}
 * 对应的方法描述符就是(IDLjava/lang/Thread;)Ljava/lang/Object
 *
 * 所有的Symbol实例通过保存在全局的SymbolTable中，SymbolTable即符号表，基于此实现符号引用计数功能。
 *
 * 当有一个新的指针指向该Symbol实例，则引用计数加1，当该指针销毁时需要将引用计数减1，当一个Symbol的引用计数为0，垃圾回收器就会从SymbolTable中删除该Symbol并回收内存。
 */
class Symbol : private SymbolBase {
 ...
 private:
    // 具有一个字节数组的属性_body，用于存储描述符对应的字符串，定义了可以操作该属性的byte_at_put(int index, int value)方法，
