...
// set_native_function用于设置native_function属性，即完成本地方法与本地代码实现的绑定
...
// clear_native_function用于将native_function属性重置为native_method_throw_unsatisfied_link_error_entry
...
// Method::clear_code方法就是编译代码即nmethod卸载的实现方法，当nmethod本身的状态变更或者CodeCache垃圾回收都会调用该方法
...
void Method::link_method(methodHandle h_method, TRAPS) {
  ...
  // 调用Interpreter::entry_for_method()函数根据特定方法类型获取到方法的入口，得到入口entry后会调用set_interpreter_entry()函数将值保存到对应属性上
...
// Method::set_code方法就是执行编译代码安装到对应的Method实例的方法
...
#else //!SHARK 如果采用非Shark即使用C1或者C2编译器
