 ...
 * 除此之外还有几个静态属性，如_first_entry、_last_entry等
 *
 * 该类定义的方法不多，大多是统计类加载性能相关的，除此之外有以下几个方法比较重要
 * 1. 加载zip文件读取写入等操作的动态链接库，load_zip_library方法
 * 2. ClassPathEntry相关的，如setup_search_path，contains_entry，add_to_list，num_classpath_entries，classpath_entry，create_class_path_entry，update_class_path_entry_list等
 * 3. 初始化的方法，initialize
 * 4. 根据类名加载指定类文件的方法，load_classfile
 */
