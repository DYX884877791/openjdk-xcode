      ...
      CASE(_putstatic):
        {
          ...
          if (cache->is_volatile()) {
            ...
            if (tos_type == itos) { // int类型赋值
             ...
             // 在JVM的字节码解释器中，如果putstatic字节码或putfield字节码的变量是java层面的volatile关键字修饰的，就会在指令执行的最后插入一道 storeLoad 屏障
             //
             // 写完值后的storeload屏障：
             //
             // 而在x86中唯一可能重排的是 读操作提前到 写操作之前，这里的storeLoad操作做的就是阻止重排
             //
