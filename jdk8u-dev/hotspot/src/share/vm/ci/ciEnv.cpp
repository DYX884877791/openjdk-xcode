...
// ------------------------------------------------------------------
// ciEnv::ciEnv
// 最终是由CompileBroker::compiler_thread_loop()调用的
// CompileBroker::compiler_thread_loop()方法就是后台编译线程执行编译的入口方法，从调用链可知每次执行一个CompileTask，就会创建一个新的ciEnv实例
...
// ciEnv::register_method用于将编译结果保存到一个nmethod中，如果是非栈上替换方法则将其安装到被编译方法上确保可以立即执行编译后的本地代码，
