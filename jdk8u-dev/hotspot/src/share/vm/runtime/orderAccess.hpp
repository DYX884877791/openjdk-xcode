...
class OrderAccess : AllStatic {
 public:
  ...
  // 重排序发生的情况，分为以下几个层次，分别为：1
  // 1. CPU 对指令的重排序、
  // 2. CPU 缓存的写入和读取导致的重排序、
  // 3. 编译器对指令的重排序


  // 可以从下面看到，到c++的实现层面，又使用c++中的volatile关键字，用来修饰变量，通常用于建立语言级别的memory barrier。
  // 在《C++ Programming Language》一书中对volatile修饰词的解释：
  //
  // A volatile specifier is a hint to a compiler that an object may change its value in ways not specified by the language so that aggressive optimizations must be avoided.
  // 含义就是：
  //
  //  1. volatile修饰的类型变量表示可以被某些编译器未知的因素更改（如：操作系统，硬件或者其他线程等）
