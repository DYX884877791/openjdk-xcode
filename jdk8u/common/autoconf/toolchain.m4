...
[
  ...
  if test "x$OPENJDK_TARGET_OS" = "xmacosx"; then
    ...
    if test "${XC_VERSION_PARTS[[0]]}" != "6" \
         -a "${XC_VERSION_PARTS[[0]]}" != "9" \
         -a "${XC_VERSION_PARTS[[0]]}" != "10" \
         -a "${XC_VERSION_PARTS[[0]]}" != "11" \
         -a "${XC_VERSION_PARTS[[0]]}" != "12" \
         -a "${XC_VERSION_PARTS[[0]]}" != "13" \
         -a "${XC_VERSION_PARTS[[0]]}" != "14" \
         -a "${XC_VERSION_PARTS[[0]]}" != "15" ; then
      AC_MSG_ERROR([Xcode 6, 9-15 is required to build JDK 8, the version found was $XCODE_VERSION. Use --with-xcode-path to specify the location of Xcode or make Xcode active by using xcode-select.])
    fi
