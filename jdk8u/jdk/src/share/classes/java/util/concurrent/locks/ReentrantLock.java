...
/**
 * ReentrantLock分为公平锁和非公平锁，并且ReentrantLock是AQS独占功能的体现
 *  　　公平锁：每个线程抢占锁的顺序为先后调用lock方法的顺序依次获取锁，就像排队一样
 *  　　非公平锁：表示获取锁的线程是不定顺序的，谁运气好，谁就获取到锁
 *
 * 我们想想，公平锁和非公平锁该如何实现：
 *  　　有那么一个被volatile修饰的标志位叫做key（其实就是上面所说的AQS中的state），用来表示有没有线程拿走了锁，还需要一个线程安全的队列，
 *     维护一堆被挂起的线程，以至于当锁被归还时，能通知到这些被挂起的线程，可以来竞争获取锁了。
...
public class ReentrantLock implements Lock, java.io.Serializable {
    ...
    abstract static class Sync extends AbstractQueuedSynchronizer {
         ...
         * 该方法会首先判断当前状态，如果c==0说明没有线程正在竞争该锁，如果c!=0 说明有线程正拥有了该锁。
         *  1.如果发现c==0，则通过CAS设置该状态值为acquires,acquires的初始调用值为1，如果CAS设置成功，则可以预计其他任何线程调用CAS都不会再成功，
         *    也就认为当前线程得到了该锁，也作为Running线程，很显然这个Running线程并未进入等待队列。
         *  2.如果c !=0 但发现自己已经拥有锁，只是简单地重新计算 status + acquires，并修改status值，但因为没有竞争，所以通过setStatus修改，
         *    而非CAS，也就是说这段代码实现了偏向锁的功能，并且实现的非常漂亮。每次线程重入该锁都会+1，每次unlock都会-1，但为0时释放锁。
         *
        ...
        final boolean nonfairTryAcquire(int acquires) {
            ...
            int c = getState();
            // 如果当前没有线程获取锁，则当前线程尝试直接获取锁，成功的话就设置一个属性exclusiveOwnerThread = current，记录锁被当前线程拿去，
    ...
    static final class NonfairSync extends Sync {
         ...
         * 非公平锁，首先是直接去获取锁，如果有并发获取失败，则在else分支中调用AQS的acquire，然后acquire中调用非公平锁的tryAcquire，进而调用nonfairTryAcquire
         *
    ...
    static final class FairSync extends Sync {
        ...
        protected final boolean tryAcquire(int acquires) {
            ...
            // 如果当前没有线程获取锁
            if (c == 0) {
                // hasQueuedPredecessors方法表示当前队列是否有线程在等待
                // 如果没有线程在等待，则采用CAS更新state的状态
            ...
            // 如果c != 0，说明已经有线程获取锁，并且getExclusiveOwnerThread == current,表示当前正在获取锁的就是当前锁，所以这里是重入！
                ...
                // 重入的话，让状态为state+acquires，即加上重入的次数
                int nextc = c + acquires;
                // 如果当前状态<0，说明出现异常
                if (nextc < 0)
                    throw new Error("Maximum lock count exceeded");
