...
//
// 重量级锁的竞争，在 ObjectMonitor::enter方法中，简单说一下下面这段代码主要做的几件事
...
void ATTR ObjectMonitor::enter(TRAPS) {
  ...
  { // Change java thread status to indicate blocked on monitor enter.
    ...
    JavaThreadBlockedOnMonitorEnterState jtbmes(jt, this);
      ...
      //设置当前monitor对象为当前线程等待的monitor对象
...
// EnterI方法会初始化线程自旋相关配置，然后自旋尝试获取锁，获取失败后将当前线程加入到ObjectWaiter队列中，然后借助底层操作系统的互斥量让当前线程处于休眠状态，
// 如果持有锁的线程释放了锁就会唤醒该线程，被唤醒后该线程会尝试获取锁，获取失败再自旋，依然获取失败再次进入休眠状态，如此循环直到获取锁为止，获取成功后将当前线程对应的ObjectWaiter从队列中移除。
//
...
//  exit用于释放锁，即将owner属性置为NULL，默认配置下会通过unpark唤醒_EntryList链表头部节点对应的等待线程，如果EntryList链表为空，
//  则将cxq链表中的元素加入到EntryList链表中且顺序保持不变，即优先唤醒最近等待的线程。注意exit方法并不会因为安全点同步而阻塞，exit方法退出后继续执行，
//  无论解释执行或者编译执行则会都被阻塞；exit方式释放锁后，被唤醒的线程占用了该锁，在enter方法获取锁准备切换线程状态时会被阻塞。
//
...
#define CHECK_OWNER()                                                                       \
  do {                                                                                      \
    if (THREAD != _owner) {                                                                 \
      /*如果owner属性不是当前线程*/                                                             \
      if (THREAD->is_lock_owned((address) _owner)) {                                        \
        /*如果owner属性位于当前线程栈帧中，说明该锁是由轻量级锁膨胀来的，则修改owner属性为当前线程*/       \
        _owner = THREAD ;  /* Convert from basiclock addr to Thread addr */                 \
        _recursions = 0;                                                                    \
        OwnerIsThread = 1 ;                                                                 \
      } else {                                                                              \
...
void ObjectMonitor::wait(jlong millis, bool interruptible, TRAPS) {
   ...
   Thread::SpinAcquire (&_WaitSetLock, "WaitSet - add") ;
    // 将ObjectWaiter节点添加到 _WaitSet队列中...
   ...
   { // State transition wrappers
     ...
     } // Exit thread safepoint: transition _thread_blocked -> _thread_in_vm 退出代码块时会切换线程状态 _thread_blocked -> _thread_in_vm
...
// notify方法是Object的notify方法的底层实现，用于“唤醒”WaitSet链表头对应的线程，即最早加入到该链表的等待线程，
...
void ObjectMonitor::notify(TRAPS) {
  ...
  // 根据调度策略选择具体哪个ObjectWaiter从_WaitSet队列中出来
...
// notifyAll方法就是Object的notifyAll方法的底层实现，对单个ObjectWaiter其处理逻辑跟notify是一致的，相比notify的实现就是增加了一个for循环，
// 会不断的从_WaitSet链表中移除头元素，然后执行notify的处理逻辑，直到_WaitSet链表为空退出循环。
...
// AddWaiter方法用于将目标ObjectWaiter加入到双向循环链表中
...
// DequeueWaiter用于移除链表头_WaitSet对应的节点，该节点是最早加入到链表的，即按照加入链表的先后顺序依次从链表中移除
...
// DequeueSpecificWaiter用于移除指定节点，不一定是_WaitSet对应的节点
