 ...
 *    最常见的转换路径，需要注意这种情况下profile可以从level 0开始，level3结束。
 *
 * b. 0 -> 2 -> 3 -> 4.
 ...
 *    当C2的编译压力较大会发生这种情形，本来应该从0直接转换成3，为了减少编译耗时就从0转换成2，等C2的编译压力降低再转换成3
 *
 * c. 0 -> (3->2) -> 4.
 ...
 *    这种情形下已经把方法加入到3的编译队列中了，但是C1队列太长了导致在0的时候就开启了profile，然后就调整了编译策略按level 2来编译，即时还是3的队列中，这样编译更快
 *
 * d. 0 -> 3 -> 1 or 0 -> 2 -> 1.
 *    After a method was once compiled with C1 it can be identified as trivial and be compiled to
 *    level 1. These transition can also occur if a method can't be compiled with C2 but can with C1.
 *    当一个方法被C1编译后，被标记成trivial的，因为这类方法C2编译的代码和C1编译的代码是一样的，所以不使用4，改成1
 *
 * e. 0 -> 4.
 ...
 *    一个方法C1编译失败且一直在收集profile信息，就从0切换到4
 *
