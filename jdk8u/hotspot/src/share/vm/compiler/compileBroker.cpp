...
void CompileBroker::init_compiler_threads(int c1_compiler_count, int c2_compiler_count) {
  ...
  char name_buffer[256];
    // 启动C2编译线程
  slog_trace("在hotspot/src/share/vm/compiler/compileBroker.cpp中的CompileBroker::init_compiler_threads函数启动了[%d]个C2编译线程...", c2_compiler_count);
  for (int i = 0; i < c2_compiler_count; i++) {
    ...
    // 启动C1编译线程
  slog_trace("在hotspot/src/share/vm/compiler/compileBroker.cpp中的CompileBroker::init_compiler_threads函数启动了[%d]个C1编译线程...", c1_compiler_count);
...
void CompileBroker::compiler_thread_loop() {
  slog_trace("进入hotspot/src/share/vm/compiler/compileBroker.cpp中的CompileBroker::compiler_thread_loop函数(编译线程启动后自动执行的函数)...");
  CompilerThread* thread = CompilerThread::current();
...
void CompileBroker::invoke_compiler_on_method(CompileTask* task) {
  slog_trace("进入hotspot/src/share/vm/compiler/compileBroker.cpp中的CompileBroker::invoke_compiler_on_method函数...");
  if (PrintCompilation) {
    ResourceMark rm;
    task->print_line();
  }
