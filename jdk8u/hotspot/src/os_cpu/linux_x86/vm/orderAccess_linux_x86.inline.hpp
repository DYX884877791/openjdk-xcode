...
// 关于这里release、acquire两个方法的语义可见：https://zhuanlan.zhihu.com/p/490465757
...
#ifdef AMD64
    // 汇编代码lock; addl $0,0(%%rsp) 其中的addl $0,0(%%rsp) 是把寄存器的值加0，相当于一个空操作（之所以用它，不用空操作专用指令nop，是因为lock前缀不允许配合nop指令使用）
    //
    // lock 汇编指令，会锁住操作的缓存行.
...
//load_ptr_acquire先将p转换成指针的指针void* const volatile *,然后再解引用操作进行取值
