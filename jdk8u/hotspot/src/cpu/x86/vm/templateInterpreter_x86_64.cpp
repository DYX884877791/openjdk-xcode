...
address InterpreterGenerator::generate_normal_entry(bool synchronized) {
  ...
  // rbx: Method* 保存方法基址
  // rcx: size of parameters
  // r13: sender_sp (could differ from sp+wordSize if we were called via c2i ) 即执行此段指令前的rsp地址(调用者的栈顶地址)，在rsp下面就是方法调用的具体入参

    //将size_of_locals地址处的此方法的本地变量个数(局部变量表最大槽数)读取到rdx中
